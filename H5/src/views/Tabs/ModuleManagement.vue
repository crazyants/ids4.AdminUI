<template>
    <el-scrollbar wrap-class="scrollbar-wrapper-y">
        <div>
            <div class="flex">
                <span>选择系统 :</span>
                <el-select size="mini" v-model='code' filterable placeholder="请选择系统">
                ...
                <div class="flex1"></div>
                <el-button type="success" size="mini" icon='el-icon-circle-plus' @click='eduitNameRole("")'>创建模块</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            eduitNameRole(row) {
                if(row){
                    this.config.title='编辑模块名称';
                    this.config.data = Object.assign({},row);
                }else {
                    this.config.title='创建模块名称';
                    this.config.data = {}
                }
